...
let
    ...
    @time for ind = 1:iterationLimit
        # Compute local observables e.g. Sz
        tmpSx = expect(ψ_copy, "Sx")
        Sx[ind, :] = tmpSx # @show (size(tmpSx), tmpSx)
        tmpSy = expect(ψ_copy, "Sy")
        Sy[ind, :] = tmpSy # @show (size(tmpSy), tmpSy)
        tmpSz = expect(ψ_copy, "Sz")
        Sz[ind, :] = tmpSz # @show (size(tmpSz), tmpSz)

        # Compute spin correlation functions e.g. Czz
        tmpCxx = correlation_matrix(ψ_copy, "Sx", "Sx", sites = 1:N)
        Cxx[ind, :] = tmpCxx[:] # @show size(tmpCxx')
        tmpCyy = correlation_matrix(ψ_copy, "Sy", "Sy", sites = 1:N)
        Cyy[ind, :] = tmpCyy[:] # @show size(tmpCyy')
        tmpCzz = correlation_matrix(ψ_copy, "Sz", "Sz", sites = 1:N)
        Czz[ind, :] = tmpCzz[:] #0 @show size(tmpCzz')
    ...
    file =
        h5open("Data/ED_N$(N)_h$(h)_Iteration$(iterationLimit)_Rotations_Only_AFM.h5", "w")
    write(file, "Sx", Sx)       # Sx
    write(file, "Sy", Sy)       # Sy
    write(file, "Sz", Sz)       # Sz
