...
let 
    ...
    # Choose a reference site and act on the initial wavefunction with the corresponding operator before doing any time evolution
    reference = 1
    reference_operator = op("Sz", s[reference])
    ψ_copy = apply(reference_operator, ψ_copy; cutoff)
    # ψ_copy = reference_operator * ψ_copy
    normalize!(ψ_copy)
    ...
    for time in 0.0 : Δτ : ttotal
        ...
        h5open("data/kicked_ising/TEBD_N$(N)_h$(h)_tau$(Δτ)_T$(ttotal).h5", "w") do file
            write(file, "Sx", Sx)
            write(file, "Sy", Sy)
            write(file, "Sz", Sz)
            write(file, "Cxx", Cxx)
    ...
    #     time_operator = op("Sz", s[site_ind])
    #     tmp_ψ = apply(time_operator, tmp_ψ; cutoff)
    #     Cxx_time[site_ind] = inner(ψ, tmp_ψ)
    # end
